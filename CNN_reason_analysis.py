...
MODELDIR="D:\\LTP_DATA\\3.3.0\\ltp_data"
...
#访问大辞林数据库
conn = mysql.connector.connect(user='root', password='password', database='bigcilin', use_unicode=True)
cursor = conn.cursor()
...
def load_embedding(path):
	...
	for line in c:
		try:
			...
			print len(word),word[0]
	return word_embedding
print "loading embedding..."
word_embedding=load_embedding("vectorsBaike.txt")
...
def load_focus(path):
	...
	for line in content:
		...
		focus[stc]=line
	return focus
focus = load_focus("svm_test_question.txt")
#test_focus = load_focus("svm_test_question_test.txt")
...
def load_answer_type(path):
	...
	for line in c:
		...
		word2 = line[4:]
		#print word2
		answer_type[word2] = word1
	return answer_type
answer_type = load_answer_type("question_with_score.txt")
test_answer_type = load_answer_type("question_with_score_test.txt")
dev_type = []
test_type = []

def load_paraphrase(path):
	paraphrase = {}
	f=open(path,"r")
	c=f.readlines()
	f.close()
	...
	for line in c:
		...
		paraphrase[key] = word
	return paraphrase
print "loading paraphrase dict..."
paraphrase = load_paraphrase("paraphrase_dic")
...
def load_test_data(path):
	...
	for line in content:
		...
		if(line[0]!=question and question != ""):
			...
			data["answer"]=answer
			type = test_answer_type[question]
			score = 0.06
			res = []
			if(type == "TIM"):
				for ans in answer:
	...
	print "answer para:",para_cnt
	print "answer hyper:",hyper_cnt
...
def CNN_train():
	...
	cnt=0
	for case in test_type:
		cnt += len(case)
	print "answer type",cnt
	#训练模型
	#y = T.lscalar()  
	x1 = T.matrix('x1')
	x2 = T.matrix('x2')
